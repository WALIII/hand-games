    ...
    def middlewrist(self):
        ...
        if results.multi_hand_landmarks:
            ...
            if hand_size > 0:
                ...
                return distance
    def handwall(self, WIDTH,HEIGHT):
        success, frame = self.cap.read()
        if not success:
            return None
        ...
        if results.multi_hand_landmarks:
            ...
            thumbtip = hand_landmarks.landmark[4].x * WIDTH
            wrist = hand_landmarks.landmark[0].y * HEIGHT
            middletip = hand_landmarks.landmark[12].y * HEIGHT
            pinkytop = hand_landmarks.landmark[20].x * WIDTH
            top = 0
            bottom = 0
            left = 0 
            right = 0
            if thumbtip > (WIDTH * 0.8):
                left = 1.5
            if wrist < (HEIGHT*0.3):
                bottom = 2
            if middletip > (HEIGHT * 0.7):
                top = 3
            if pinkytop < (WIDTH * 0.2):
                right = 4
            if top > 0:
